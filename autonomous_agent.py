...
def attach_to_project(project_path: Path) -> bool:
    ...
    try:
        ...
        # Create startup script using a relative path so it works if the
        # agent file is moved (falls back to the absolute path at write time).
        agent_abs = Path(__file__).absolute()
        try:
            agent_rel = agent_abs.relative_to(project_path.absolute())
            agent_ref = f'Path(__file__).parent / r"{agent_rel}"'
        except ValueError:
            # agent lives outside the project tree — use absolute path
            agent_ref = f'Path(r"{agent_abs}")'
